        ...
        List<long> packIds, ChunkingProfile profile, string lease, CancellationToken cancellationToken)
    {
        ...
        foreach (PackChunkCandidate candidate in candidates.Values)
        {
            ...
            if (result.Inserted)
            {
                ...
                {
                    ContentChunkId = result.Id, StoragePackId = candidate.PackId,
                    PackOffset = candidate.Entry.Offset, StoredLength = candidate.Entry.StoredLength,
                    RawLength = candidate.Length, CompressionAlgorithm = "Zstd", CompressionVersion = 1,
                    StorageFormatVersion = 1, CreatedAt = timeProvider.GetUtcNow()
                });
        ...
        foreach (ArchivePackPlan plan in plans)
        {
            ...
            {
                TenantId = item.TenantId, ManifestFormatVersion = 1, HashAlgorithm = "SHA-256",
                OriginalHash = item.Sha256Hash!, OriginalSizeBytes = item.SizeBytes,
                ChunkingAlgorithm = profile.Algorithm, ChunkingAlgorithmVersion = profile.AlgorithmVersion,
                MinimumChunkSizeBytes = profile.MinimumSizeBytes, AverageChunkSizeBytes = profile.AverageSizeBytes,
                MaximumChunkSizeBytes = profile.MaximumSizeBytes, ChunkCount = plan.Chunks.Count,
                CommittedAt = timeProvider.GetUtcNow()
            };
            foreach (PlannedChunk chunk in plan.Chunks)
                manifest.Chunks.Add(new ManifestChunk
                {
                    SequenceNumber = chunk.Sequence, OriginalOffset = chunk.Offset, RawLength = chunk.Length,
