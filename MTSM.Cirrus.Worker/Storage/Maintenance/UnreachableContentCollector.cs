...
public sealed class UnreachableContentCollector(IServiceScopeFactory scopeFactory)
{
    public async Task PruneAsync(CancellationToken cancellationToken)
    {
        ...
        await db.ContentManifests.Where(x => !db.ArchiveObjects.Any(a => a.ContentManifestId == x.ContentManifestId))
            .ExecuteDeleteAsync(cancellationToken);
        await db.StorageLocations.Where(x => !db.ManifestChunks.Any(m => m.ContentChunkId == x.ContentChunkId))
            .ExecuteDeleteAsync(cancellationToken);
        await db.ContentChunks.Where(x => !db.ManifestChunks.Any(m => m.ContentChunkId == x.ContentChunkId))
            .ExecuteDeleteAsync(cancellationToken);
        await db.StoragePacks.Where(x => x.PackStatus == PackStatus.Committed && !x.StorageLocations.Any())
